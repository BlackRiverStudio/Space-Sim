...
namespace Wakaba.AI
{
    public class Spawner : MonoBehaviour
    {
        ...
        [SerializeField] private Minable bossPrefab = null;
        [SerializeField] private Minable enemyPrefab = null;
        ...
        public void Spawn()
        {
            Minable prefab = shouldSpawnBoss && Random.Range(0, 100) < bossSpawnChance ? bossPrefab : enemyPrefab;
            Vector3 position = transform.position + new Vector3(Random.Range(-size.x * 0.5f, size.x * 0.5f), floorYPosition ? 0 : Random.Range(-size.y * 0.5f, size.y * 0.5f), Random.Range(-size.z * 0.5f, size.z * 0.5f)) + centre;

            position = transform.InverseTransformPoint(position);

            // Instantiate(prefab, position, transform.rotation, transform);
            pool.Spawn(prefab, position, Quaternion.identity, transform);
            if (prefab == null)
            {
